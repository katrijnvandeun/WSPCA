...
for (teller in (1:720))
{
	DATA<-read.table(paste('../DATA/DATAnomissc',teller,'.dat',sep=""))
	TRUEDATA<-read.table(paste('../DATA/TRUEnomissc',teller,'.dat',sep=""))
	TRUEd<-as.matrix(TRUEDATA)
	W<-read.table(paste('../DATA/WEIGHTSnomissc',teller,'.dat',sep=""))
	DATAmiss<-as.matrix(DATA)
	DATAmiss[W==0]<-NaN
	DATAmiss[is.infinite(DATAmiss)]<-NaN
	PTRUE<-read.table(paste('../DATA/TRUEPnomissc',teller,'.dat',sep=""))
	PTRUE<-as.matrix(PTRUE)
	TTRUE<-read.table(paste('../DATA/TRUETnomissc',teller,'.dat',sep=""))
	...
	RESULT<-rbind(RESULT,c(presstrueiw,spcr_tucongrT,spcr_tucongrP))
}
write.table(RESULT,file="../RESULTS/PMDresultPunif.txt",sep="\t",eol="\n")
#######


#RESULT<-read.table("../RESULTS/PMDresultPunif.txt",sep="\t")

######BOX PLOTS ##############


#load PCA,wPCA,wSPCA results
matlabresults=read.table("RESULTnomiss_sqrtW_Puniof.txt",sep="\t")
...
for (sparse in 1:2) {
  for (i in 1:2) {
    for (j in 1:2) {
      nonsparse <- which(matlabresults[,4]==SPlevels[sparse])
...
Level_I <-Ilevels[i]#'100'
Level_J <-Jlevels[j]
#  
pdf(paste("boxplot_Prandom_sparse",SPlevels[sparse],"_congruenceP_I",Level_I,"_J",Level_J,".pdf",sep = ""),width=12,height = 8,paper="special")
par(mfrow=c(1,3))
#
...
press <- log(press+1)
m <- min(press)
M <- ifelse(sparse!=1,max(press),2) 
#
pdf(paste("boxplot_Prandom_sparse",SPlevels[sparse],"_pressP_I",Level_I,"_J",Level_J,".pdf",sep = ""),width=12,height = 8,paper="special")
par(mfrow=c(1,3))
#
